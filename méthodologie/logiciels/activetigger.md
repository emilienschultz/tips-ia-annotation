...
*Le logiciel peut être déployé à partir de docker, ou installé en ligne de commande (back-end et front-end séparés). Il y a une  instance en ligne hébergée (sur demande de compte) ; un GPU est recommandé pour l'entraînement des modèles.*
...
<!-- TODO: LM : j'inverserai instance de démo en ligne (en ajoutant le lien formulaire) et installation autonome pour pas faire peur aux néophytes -->
...
<!-- TODO: LM : tu évoques plus haut la fonctionalité images, soit la virer en haut, ou ajouter / clarifier ici pour pas avoir info contradictoire -->

## Interopérabilité

*Import et export tabulaires classiques ; les annotations, prédictions et les poids des modèles affinés sont exportables.*
...
<!-- TODO: LM : aviser si plus clair visuellement (ou pas) par une liste comme dans d'autres fiches je crois . Genre :
L'export couvre (aux format de sortie Parquet ou CSV):
- les annotations (jeu complet ou ensembles train/test, avec les colonnes d'origine et une colonne par schéma de codage),
- les prédictions du modèle sur l'ensemble du corpus ou sur un jeu externe,
- les features et les coordonnées de projection issues de la visualisation,
- les poids du modèle BERT affiné (réutilisable en dehors du logiciel).
Pas sur que ce soit mieux, mais au cas où.
-->
...
La documentation officielle est en anglais ([https://activetigger.com/documentation/](https://activetigger.com/documentation/)). Un serveur Discord sert à l'entraide et les issues GitHub au signalement de bugs. Des présentations existent en français (Vendredis Quanti, séminaires) et en anglais (PyData 2025, vidéo YouTube). Plusieurs publications francophones en sciences sociales utilisent le logiciel (Actes de la recherche en sciences sociales, Revue française de science politique, Sciences sociales et santé), signe d'une diffusion dans les réseaux de recherche français.

<!-- TODO: LM: aviser si gagne signe en diminuant les parenthèses (présentations, publis), quitte a pointer directement vers certains lignes spécifiques ou si vous avez des pages recensent les supports -->

## Collaboratif

*Conçu comme collaboratif : plusieurs annotateurs sur un même projet avec une gestion des rôles.*

Le logiciel permet de collaborer à plusieurs sur un projet et donc d'organiser des campagnes d'annotation en équipe. La gestion des utilisateurs distingue quatre rôles : root (administration de l'instance), manager (créateur du projet, tous droits), collaborator (comme manager sans la suppression) et annotator (accès limité au codebook, à l'exploration et à l'annotation). Les aspects collaboratifs ne sont pas encore bien documentés.
...
Active Tigger est pensé autour de la boucle humain-modèle : l'annotation manuelle sert à entraîner des modèles (classifieurs rapides puis BERT affiné) qui à leur tour priorisent et accélèrent l'annotation (active learning). Ces fonctions restent pilotées par l'utilisateur, qui garde la main sur les annotations.  
Une page « Generative », optionnelle et expérimentale permet aussi d'annoter ou d'extraire de l'information par prompts en connectant des modèles externes (OpenAI, OpenRouter, Ollama, HuggingFace ou services académiques). L'utilisateur choisit le modèle, l'endpoint et fournit son propre jeton d'accès. À noter que les LLM externes s'exécutent sur des serveurs tiers, ce qui pose les questions habituelles de confidentialité des données.

<!-- TODO: LM : alternatives moins précise pour premier paragraphe, mais possiblement plus grand public (pas certain) :

Active Tigger est pensé autour de la boucle humain-modèle : les annotations manuelles servent à entraîner des modèles, qui aident ensuite à repérer les textes les plus utiles à annoter (active learning). Ces fonctions restent pilotées par l'utilisateur, qui garde la main sur les annotations.

2eme paragraphe : possible de virer si besoin alléger
- "(OpenAI, OpenRouter, Ollama, HuggingFace ou services académiques)"-> seulement si on veut alléger. En gros au pire la précision les utilisateurs avancés trouveront l'info, les autres seront pas plus éclairés. Je pense qu'en l'état c'est bien, mais je fais trop long souvent :) 
 -->
